...
public class DefaultStateConfigurer<S, E>
	...
	public StateConfigurer<S, E> state(S state, Collection<? extends Action<S, E>> stateActions) {
		...
		if (stateActions != null) {
			rStateActions.addAll(stateActions.stream().map(Actions::from).collect(Collectors.toList()));
		}
	...
	public StateConfigurer<S, E> state(S state, Collection<? extends Action<S, E>> entryActions,
		...
		if (entryActions != null) {
			...
			rEntryActions.addAll(entryActions.stream().map(Actions::from).collect(Collectors.toList()));
		}
		if (exitActions != null) {
			rExitActions = new ArrayList<>();
			rExitActions.addAll(exitActions.stream().map(Actions::from).collect(Collectors.toList()));
		}
