...
public class StateContextTests extends AbstractStateMachineTests {
	...
	Arrays.asList(Stage.TRANSITION_START,Stage.EXTENDED_STATE_CHANGED,Stage.TRANSITION,Stage.STATE_ENTRY,Stage.TRANSITION_START,Stage.TRANSITION,Stage.STATE_ENTRY,Stage.TRANSITION_START,Stage.TRANSITION,Stage.STATE_ENTRY,Stage.STATE_CHANGED,Stage.STATEMACHINE_START,Stage.TRANSITION_END,Stage.STATE_CHANGED,Stage.STATEMACHINE_START,Stage.TRANSITION_END,Stage.STATE_CHANGED,Stage.STATEMACHINE_START,Stage.TRANSITION_END
	).toArray(new Stage[0])
		);
	...
	public void testEventNotAccepted() throws Exception {
		...
		assertThat(listener.contexts.stream().map(c -> c.getStage()).collect(Collectors.toList())).containsExactly(
	Arrays.asList(Stage.EVENT_NOT_ACCEPTED,Stage.EVENT_NOT_ACCEPTED,Stage.EVENT_NOT_ACCEPTED
	).toArray(new Stage[0])
		);
